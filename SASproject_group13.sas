...
* Choose the appropriate path (different for each group member);
...
proc print data=work.pollution (obs=10)noobs; run;
proc print data=work.climate (obs=10) noobs; run;
...
* Prepare the ods settings for pdf output;
...
p "Ever since the industrialization of the world and the enormous increase in the global human population it brought with it, 
    pollution has increasingly become a problem. The rising industrial and energy production relied on the burning of fossil 
    fuels and biomass to operate and made use of environmentally toxic chemicals that were released in the air and gravely 
    deteriorate the quality of the air we breath. The expansion of towns and cities, and the dramatic increase in traffic only 
    aggravated the air pollution problem. Realizing the problem, governments all over the world, both individually and together, 
    vouched to create standards, start monitoring and increase air quality in, for example, the Clean Air Act (CAA) by the US, 
    the Montreal Protocol by the United Nations, and the Convention on Long-range Transboundary Air Pollution (Air Convention) 
    by the European Commission.";
p 	"According to the World Health Organisation (WHO), 9 out of 10 people breathe air containing high levels of pollutants and 
	this air pollution kills an estimated seven million people worldwide every year. The causes of these premature deaths are strokes, 
	...
	sulphur dioxide (SO2) and carbon monoxide (CO). The Environmental Protection Agency (EPA) has developed an Air Quality Index (AQI) based on these 
	pollutants. It is an index for reporting the daily air quality and is divided in six categories indicating levels of health concern, ranging from 
	good (0-50) to hazardous (301 – 500) air quality conditions. ";
p	"Ground-level ozone, not to be confused with atmospheric ozone, and airborne particulate matter pose the greatest threat to human health in the US. 
	Ozone at ground level is one of the major constituents of photochemical smog. It is formed by the reaction with sunlight (photochemical reaction) of 
	pollutants such as nitrogen oxides (NOx) and volatile organic compounds (VOCs) emitted by vehicles, solvents and industry. As a result, the highest 
	levels of ozone pollution occur during periods of sunny weather. Excessive ozone in the air can have a marked effect on human health. It can cause 
	breathing problems, trigger asthma, reduce lung function and cause lung diseases. ";
p	"In this project we focus on the ozone levels in the state of Pennsylvania (PA) in the US between the years 2000 and 2016.";
run;
