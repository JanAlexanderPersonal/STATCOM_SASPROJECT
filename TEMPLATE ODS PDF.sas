...
* Choose the appropriate libname (different for each group member);
...
/* also doable with proc sql */
proc sql;
create table Proj.pollutionTemp20152 as
select mean(O3_AQI), mean(Temperature_Mean)
	from Proj.pollutionTemp
	where year(Date_Local) = 2015
	group by county ,month(Date_Local);

proc print data = Proj.pollutionTemp20152; run;
